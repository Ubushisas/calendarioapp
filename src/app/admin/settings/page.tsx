...
function SortableRoomItem({ roomKey, room, updateSetting, onRename, onDelete, openMenu, setOpenMenu }: any) {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  return (
    ...
    >
      ...
      <div className="flex-1">
        ...
        <p className="text-xs text-gray-500 mt-1">ID: {roomKey}</p>
      </div>
      <label className="relative inline-flex items-center cursor-pointer">
        <input
          type="checkbox"
          checked={room.enabled}
          onChange={(e) => updateSetting(`rooms.${roomKey}.enabled`, e.target.checked)}
          className="sr-only peer"
        />
      ...
      <div className="relative">
        <button
          onClick={() => setOpenMenu(openMenu === roomKey ? null : roomKey)}
          className="p-2 hover:bg-gray-200 rounded-lg transition"
        >
          <MoreVertical className="w-5 h-5 text-gray-600" />
        </button>
...
export default function SettingsPage() {
  ...
  const [modalRoomCalendarId, setModalRoomCalendarId] = useState('')
  ...
  useEffect(() => {
    if (settings?.rooms) {
      setRoomOrder(Object.keys(settings.rooms))
    }
  }, [settings?.rooms])
  ...
  const addRoom = () => {
    setModalRoomName('')
    setModalRoomCalendarId('')
    setShowAddRoomModal(true)
  }

  const confirmAddRoom = () => {
    if (!modalRoomName || modalRoomName.trim() === '') return
    if (!modalRoomCalendarId || modalRoomCalendarId.trim() === '') {
      alert('Por favor ingresa un ID de calendario de Google')
      return
    }

    const roomId = modalRoomName.toLowerCase().replace(/\s+/g, '_')
    ...
    setSettings((prev: any) => ({
      ...
      rooms: {
        ...
        [roomId]: {
          enabled: true,
          name: modalRoomName,
          calendarId: modalRoomCalendarId
        }
      }
    }))
  ...
  const deleteRoom = (roomKey: string) => {
    if (!confirm(`¿Estás seguro de eliminar la sala "${settings.rooms[roomKey].name}"?`)) return
  ...
  const renameRoom = (roomKey: string) => {
    setModalRoomId(roomKey)
    setModalRoomName(settings.rooms[roomKey].name)
    setShowRenameRoomModal(true)
    setOpenMenuRoom(null)
  }

  const confirmRenameRoom = () => {
    if (!modalRoomName || modalRoomName.trim() === '' || modalRoomName === settings.rooms[modalRoomId].name) return
  ...
  const handleRoomDragEnd = async (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      const oldIndex = roomOrder.indexOf(active.id as string)
      const newIndex = roomOrder.indexOf(over.id as string)

      const newOrder = arrayMove(roomOrder, oldIndex, newIndex)
      setRoomOrder(newOrder)

      // Reorder rooms object
      const newRooms: any = {}
      newOrder.forEach((key) => {
        newRooms[key] = settings.rooms[key]
      })

      const newSettings = { ...settings, rooms: newRooms }
      ...
      try {
        ...
        console.error('Failed to auto-save reordered rooms:', error)
      }
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className={`grid gap-6 ${showPreview && !isMobile ? 'grid-cols-2' : 'grid-cols-1'}`}>
          {/* Settings Panel */}
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6">
              <div className="flex items-center justify-between gap-4">
                <div className="flex-1">
                  <h2 className="text-base sm:text-lg font-semibold text-gray-900">Estado del Sistema</h2>
                  ...
                  >
                    {roomOrder.map((roomKey) => (
                      <SortableRoomItem
                        key={roomKey}
                        roomKey={roomKey}
                        room={settings.rooms[roomKey]}
                        updateSetting={updateSetting}
                        onRename={renameRoom}
                        onDelete={deleteRoom}
                        openMenu={openMenuRoom}
                        setOpenMenu={setOpenMenuRoom}
                      />
                    ))}
                  </SortableContext>
                </DndContext>
      ...
      {showAddRoomModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-2xl max-w-md w-full p-6 animate-in fade-in zoom-in duration-200">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Nueva Sala</h3>
            <div className="space-y-4">
              <div>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">ID de Google Calendar</label>
                <input
                  type="text"
                  value={modalRoomCalendarId}
                  onChange={(e) => setModalRoomCalendarId(e.target.value)}
                  placeholder="c_xxxxxxxxxxxxx@group.calendar.google.com"
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-black focus:border-transparent"
                />
                <p className="text-xs text-gray-500 mt-1">El ID del calendario de Google Calendar asociado a esta sala</p>
              </div>
            </div>
            <div className="flex gap-3 mt-6">
              <button
      ...
      {showRenameRoomModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-2xl max-w-md w-full p-6 animate-in fade-in zoom-in duration-200">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Renombrar Sala</h3>
            <input
              type="text"
              value={modalRoomName}
              onChange={(e) => setModalRoomName(e.target.value)}
              onKeyDown={(e) => e.key === 'Enter' && confirmRenameRoom()}
              placeholder="Nuevo nombre"
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-black focus:border-transparent"
              autoFocus
            />
